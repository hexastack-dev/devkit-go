...
func TestStackdriverLog(t *testing.T) {
	...
	ctx, span := otel.Tracer("stackDriverTest").Start(context.Background(), "test")
	defer span.End()
	...
	if err := json.Unmarshal(got, &r); err != nil {
		...
		if got, want := jsonString(r, "logging.googleapis.com/tracer"), ent.TraceID.String(); got != want {
...
func BenchmarkE2E(b *testing.B) {
	...
	b.Run("WithLog", func(b *testing.B) {
		l := NewStackdriverLogger(io.Discard, func(error) {})
		run(b, New(l)(http.HandlerFunc(benchHandler)))
	})
}
